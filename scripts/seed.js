...
async function seedCredentials(client) {
  try {
    ...
    const insertedUsers = await Promise.all(
      credentials.map(async (user) => {
        const hashedPassword = await bcrypt.hash(user.password, 10);
        return client.sql`
        INSERT INTO credentials (id, name, email, password)
    ...
    console.log(`Seeded ${insertedUsers.length} credentials`);
  ...
  try {
    ...
    const insertedStudents = await Promise.all(
      students.map(async (student) => {
        ...
        VALUES (${student.name}, ${student.xp})
        ON CONFLICT (id) DO NOTHING;
      `;
      }),
    );
